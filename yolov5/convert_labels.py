...
labels = json.load(open('./datasets/train_annotations'))
for i in labels:
    image_id = i['image_id']
    bbox = i['bbox']
    ...
    image_path = os.path.join('./datasets/train', image_name)
    ...
    with open(os.path.join('./datasets/train', txt_name), 'w') as f:
        f.write(string)
        print("Convert {}".format(txt_name))
        f.close()

labels = json.load(open('./datasets/valid_annotations'))
for i in labels:
    image_id = i['image_id']
    bbox = i['bbox']
    category_id = str(i['category_id'] - 1)
    ...
    image_name = 'image_id_' + str(image_id).zfill(3) + '.jpg'
    image_path = os.path.join('./datasets/valid', image_name)
    image = cv2.imread(image_path)
    height, width, _ = image.shape
    yolo_bbox = convert_coco_bbox_to_yolo(bbox, (width, height))
    ...
    string = category_id + ' ' + ' '.join([str(a) for a in yolo_bbox]) + '\n'
    txt_name = image_name.replace('.jpg', '.txt')
    with open(os.path.join('./datasets/valid', txt_name), 'w') as f:
        f.write(string)
        print("Convert {}".format(txt_name))
        f.close()
